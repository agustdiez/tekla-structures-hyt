...
Para crear un nuevo dibujo debemos dirigirnos al [Document Manager](generalidades_dibujo.md#document-manager), y luego haciendo click en el apartado de `Create GA drawing` se creará la hoja.
...
Otra forma de acceder a las propiedades es a travez del Ribbon del model:

![alternativas para dibujos](../img/dibujo/alternativas%20para%20dibujos.png)

1. Podremos acceder a las [Propiedades de GA drawing](generalidades_dibujo.md#propiedades-de-ga-drawing) y al ["Layout editor"](../reportes/cuadro_rotulo.md).

2. Podremos ingresar al `Master drawing catalog` y crear un dibujo en base a alguna vista que tengamos:
    - El `Master drawing catalog` permite crear dibujos en base a un "dibujo maestro", donde los nuevos dibujos serán igual a este. Recomendamos usar:
...
{: .note}
> Deben completarse los campos de Name; Title 1, Title 2; Title 3. Para mas desarrollo sobre los atributos, referir a [Cuadros del rotulo](../reportes/cuadro_rotulo.md)
...
- En `(2)` tenemos las disposiciones que se fueron creando en cada proyecto, pudiendo ser modificadas clickeando el botón `Edit` e ingresando al ["Layout editor"](../reportes/cuadro_rotulo.md) y definir el tamaño de hoja que se desea `(3)`,
...
{: .note}
> También dentro del apartado "Views" puede configurarse la escala que se quiere dejar configurada para cada una de las vistas que se creen, esto sujeto a que  al momento del dibujo pueda modificarse a la que se desee, partiendo de la que se asigne en esta instancia.
...
En el apartado `Annotations` se encuentran distintas marcas `(1)`, como las ["part marks"](./marcas_simbolos_notas.md#part-mark) utilizadas en estructura metálica y armaduras, [marcas de soldadura](./marcas_simbolos_notas.md#weld-mark), [niveles](./marcas_simbolos_notas.md#level-mark), [revisión](./marcas_simbolos_notas.md#revision-mark), etc. También como útil encontramos la opción de poner [símbolos](./marcas_simbolos_notas.md#simbolos) ya pre-configurados.  Luego encontramos las opciones de [escritura](./marcas_simbolos_notas.md#textos)`(2)`.

{: .warning}
>No se suele utilizar la marca de sección ni la marca de detalle por el hecho de que se usa en el apartado views.
...
{: .highlight}
> Recomendación, al seleccionar las teclas (Ctrl+F) se puede hacer una acotación libre, y al seleccionar (Ctrl+G) se puede realizar una acotación ortogonal.

![ribbon view](../img/dibujo/ribbon%20view.png)

Por último el apartado de ["Views"](generalidades_dibujo.md#views) nos permite trasladar a nuestro dibujo los elementos de modelo que deseamos representar. Se suele utilizar el ["Detail view"](./vistas_dibujo.md#detail-view), ["Section view"](./vistas_dibujo.md#section-view), ["Area in model view"](./vistas_dibujo.md#area-in-model-view), ["Entire model view"](./vistas_dibujo.md#entire-model-view), ["Area in drawing view"](./vistas_dibujo.md#area-in-drawing-view), etc.

### Document Manager

El document manager almacena los diferentes dibujos que se fueron creando, y sus documentos. Dentro de estos se pueden ver las celdas con la información de cada dibujo, con sus propiedades y atributos definidos por el usuario, fecha de creación, etc.
...
Dentro de las propiedades que pueden modificarse de un GA drawing se encuentran:
- **User-defined attributes**: Son parámetros que completa el usuario que realiza el dibujo. Suelen tener que completarse según los atributos que se necesiten los cuadros que haya que completar. Los valores disponibles para completar son los siguientes:
![Atributos definidos por el usuario](../img/dibujo/Atributos%20definidos%20por%20el%20usuario.png)
Se recomienda leer tanto la explicación del ["Editor de cuadros"](../reportes/editor_cuadros.md), como la del ["Layout editor"](../reportes/cuadro_rotulo.md).

- **Revisión**: Este conjunto de parámetros se verá reflejado en los cuadros dentro del "Layout editor" donde se indique en que revisión se está trabajando, además también en las [Revisión marks](../dibujo/marcas_simbolos_notas.md#revision-mark).

    ![revisión atributos](../img/dibujo/revisión%20atributos.png)
    - En `(1)` tendremos guardados la configuración con los valores que suelen utilizarse dentro de los rótulos.
    - En `(2)` colocaremos la revisión que estemos trabajando, puede ser A,0,1,etc.
    - En `(3)` indicaremos las personas que formaron parte del proceso del documento, siendo que `Created by` representa al ejecutors del plano, en `Checked by` el revisor (LEP) y en `Approved by` irá el lider del proyecto.
...
Una vez completados los valores que se requieran, se debe seleccionar el botón `Create`. Luego aparecerá la última revisión que creamos y en caso de crear otra, aparecerá esta nueva de manera progresiva, logrando así tener revisiones consecutivas.

![Ejemplo de crear revisión](../img/dibujo/Ejemplo%20de%20crear%20revisión.gif)
*Figura 8: Ejemplo de crear revisión*.

- **Copy drawing views/with layout**: Se utiliza para copiar un dibujo que se encuentra en un "GA Drawing" hacia otro, además de poderse hacer con o sin el layout.

    ![Copy drawing views](../img/dibujo/Copy%20drawing%20views.gif)

- **Lock**: Se utiliza para marcar que un dibujo no está disponible para su edición. Se coloca `On` o `Off`.

- **Freeze**: Se utiliza para pausar la inteligencia del dibujo. Si se hace alguna modificación en el model, se modificarán los elementos en las vistas del dibujo pero no las marcas, cotas, etc.

- **Ready for issuing**: Se utiliza para marcar cuando un dibujo está listo para su emisión. Si se modifica algo en el model también se modificará aquí.

- **Issue**: Se utiliza para indicar la emisión del plano.
...
Las propiedades que permiten modificarse del GA drawing desde el modo dibujo, no son las mismas que las que pueden modificarse desde el model, ya que aquí las opciones serán las siguientes:
...
- **Recreate**:Se utiliza para recrear el dibujo en caso de haber realizado algún ajuste que no se vea reflejado en el mismo. Para evitar que suceda automaticamente puede usarse "Freeze".

- **Print**:Se utiliza para [imprimir](impresion_exportacion.md#print-drawings-pdf) el dibujo.

- **Export**: Se utiliza para [exportar](impresion_exportacion.md#export-drawings-dwg) nuestro dibujo a formato DWG/DXF.

- **Copy rows to Clipboard**: Se utiliza para copiar las propiedades del documento en el portapepeles, y puede pegarse luego en otros programas como Excel.
![Copy rows to Clipboard](../img/dibujo/Copy%20rows%20to%20Clipboard.gif)

- **Select all**: Se utiliza para seleccionar todos los elementos que hayan en el document manager (según si se está en "All documents", "All files", etc.)

- **Add to Master drawing catalog**: Permite añadir el drawing actual al  [Master drawing catalog](generalidades_dibujo.md#crear-nuevo-ga-drawing).

- **Copy to new sheet**: Se utiliza para copiar nuestro drawing a uno nuevo con las mismas características. Temporalmente aparecerá como "Drawing was cloned".

- **Delete**: Se utiliza para eliminar un drawing.

### Panel lateral derecho: 
Dentro de este panel tenemos 5 herramientas útiles:

![panel lat derecho](../img/dibujo/panel%20lat%20derecho.gif)
*Figura 10:  panel lateral derecho*.
...
Con `(2)` podremos añadir como detalle a una parte del dibujo que hayamos creado, perderá sus propiedades y pasará a ser como una imagen que podremos usar en otros modelos.

![Libreria 2d ejemplo](../img/dibujo/Libreria%202d%20ejemplo.gif)
*Figura 11: `Libreria 2D` ejemplo*.

#### Instructor
El objetivo principal del instructor es brindar apoyo a la hora de usar una herramienta. Funciona principalmente para herramientas del [ribbon](generalidades_dibujo.md#descripción-del-modo-dibujo). Esta función tambien explica las funciones en el modelado en general. 
...
- También se puede acceder a foros de discusión [Tekla Discussion Forum](https://forum.tekla.com/?utm_source=prod-tekla-structures&utm_medium=feat-sidepane). 
...
*Figura 15:  `Applications & components`*

### Barra de herramientas inferior

![Barra de herramientas inferior](../img/dibujo/Barra%20de%20herramientas%20inferior.png)

- En `(1)` podremos elegir que elementos de dibujo podrán ser o no seleccionados, además de poder agregar un filtro de selección desde aquí. [No puedo seleccionar nada](../faq/faq.md#no-puedo-seleccionar-nada)
- En `(2)` elegiremos los "Snaps" que queremos utilizar al dibujar, como el "Intersection points", el "Mid points", etc.
- En `(3)` podemos elegir algún Snap temporal, por ejemplo si no tuviese seleccionado el "Mid point" pero necesito en un lugar ese punto hago click primero en la herramienta que voy a seleccionar en el punto y luego en ![mid point temporal](../img/dibujo/mid%20point%20temporal.png)
...
- `(1)`: Sirve para guiar en el uso de las distintas herramientas del programa. 
    ...
    Puede indicar que hagas un click en algún lugar ![pick first point on line](../img/dibujo/pick%20first%20point%20on%20line.png), y luego que hagas otro ![pick second point on line](../img/dibujo/pick%20second%20point%20on%20line.png)según la herramienta que esté en uso. 

- `(2)`: Nos avisa si tenemos activa la ortogonalidad de las lineas (clickeando con la letra `O`).

- `(3)`: Seleccionando `Mayús + M` podemos alternar entre la forma de trasladarnos "PAN" (más usada) o "SCROLL", donde el primero se traslada con el click del medio fijando un punto, mientras que la otra opción con el mismo click se traslada en la dirección que tenga el cursor después del click.
...
![menú settings](../img/dibujo/menú%20settings.png)
    ...
    > En `Advanced options` es importante que se tenga la siguente configuración para crear correctamente las "Draw Clouds o nubes de revisión":![Draw Clouds](../img/dibujo/Draw%20Clouds.png)
